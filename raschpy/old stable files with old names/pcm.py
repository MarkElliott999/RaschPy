...
class PCM(Rasch):
    ...
    def __init__(
        self, dataframe, max_score_vector=None, extreme_persons=True, no_of_classes=5
    ):
            ...
            self.max_score_vector = pd.Series(
                {
                    ...
                    for item, max_score in zip(
                        dataframe.columns, dataframe.max().to_numpy()
                    )
                }
            )
        else:
        ...
        if extreme_persons:
            ...
            max_scores = (
                dataframe.notna().mul(self.max_score_vector, axis=1).sum(axis=1)
            )
    ...
    def calibrate_anchor(
        ...
        self.central_diffs_anchor = (
            self.central_diffs.copy() + self.anchor_trans_constant
        )
    ...
    def std_errors(
        ...
        """
        samples = [
            PCM(self.dataframe.sample(frac=1, replace=True), self.max_score_vector)
        ...
        calibrations_thresholds = {
            item: np.stack(
                [samples[s].thresholds_uncentred[item] for s in range(no_of_samples)]
            )
            for item in self.items
        }
        calibrations_central = {
            item: np.array(
                [samples[s].central_diffs[item] for s in range(no_of_samples)]
            )
            for item in self.items
        }
        ...
        self.cat_width_se = {
            item: np.std(self.cat_width_bootstrap[item], axis=0) for item in self.items
        }
    ...
    def abil(
        ...
        try:
            ...
            while active.any() and iters <= max_iters:
                active_persons = (
                    persons[active]
                    if hasattr(persons, "__getitem__")
                    else [p for p, a in zip(persons, active) if a]
                )
            ...
            warnings.warn(
                f"abil() failed with exception: {e}. " "Returning NaN for all persons.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def score_abil(
        ...
        while abs(change) > tolerance and iters <= max_iters:
            result = sum(
                self.exp_score_uncentred(estimate, thresholds[item]) for item in items
            )
            info = sum(
                self.variance_uncentred(estimate, thresholds[item]) for item in items
            )
    ...
    def fit_statistics(
        ...
        self.exp_score_df = pd.DataFrame(
            exp_score, index=abilities.index, columns=self.items
        )
        ...
        self.kurtosis_df = pd.DataFrame(
            kurtosis, index=abilities.index, columns=self.items
        )
        ...
        (self.point_measure, self.exp_point_measure) = self.pt_meas(
            self.person_abilities, self.exp_score_df, self.info_df
        )
                ...
                item: {
                    ...
                    + 1: (
                        (
                            ...
                            - self.thresholds_uncentred[item].iloc[t]
                        )
        ...
        if test_stats:
            ...
            self.isi_central = (
                self.central_diffs.var() / (self.item_se**2).mean() - 1
            ) ** 0.5
            ...
            self.psi = (
                np.var(self.person_abilities) - (self.rsem_vector**2).mean()
    ...
    def threshold_stats_df(
        ...
        estimate_array = np.concatenate(
            [self.thresholds_uncentred[item].values for item in self.dataframe.columns]
        )
    ...
    def save_stats(
        ...
        if format == "xlsx":
            ...
            with pd.ExcelWriter(
                ...
                self.threshold_stats_centred.to_excel(
            ...
            self.threshold_stats_centred.to_csv(
                f"{filename}_threshold_stats_centred.csv"
            )
    ...
    def class_intervals(self, abilities, items=None, no_of_classes=5):
        ...
        mask_dict = {
            ...
            **{
                f"class_{i + 2}": (
                    (abils >= quantiles.values[i]) & (abils < quantiles.values[i + 1])
                )
                for i in range(no_of_classes - 2)
            },
        }
        mean_abilities = pd.Series(
            {cg: abils[mask_dict[cg]].mean() for cg in class_groups}
        )
        obs = pd.concat(
    ...
    def class_intervals_thresholds(self, item, no_of_classes=5):
        ...
        for t in range(self.max_score_vector[item]):
            ...
            mean_abilities.append(
                [combined["abil"][masks[cg]].mean() for cg in class_groups]
            )
            obs_props.append(
        ...
        with plt.rc_context({"font.family": font, "font.size": axis_font_size}):
            ...
            if items is not None:
                if isinstance(items, str) and items not in ("all", "none"):
                    ...
                    thresholds = {
                        item: self.thresholds_uncentred[item] for item in items
                    }
            else:
                thresholds = self.thresholds_uncentred
            ...
            if point_info_lines_item[1] is not None:
                ...
                for abil in point_info_lines_item[1]:
                    info = self.variance_uncentred(
                        abil, self.thresholds_uncentred[item]
                    )
                    ax.vlines(
            ...
            if point_info_lines_test is not None:
                ...
                for abil in point_info_lines_test:
                    info = sum(
                        self.variance_uncentred(abil, self.thresholds_uncentred[it])
                        for it in item_keys
                    )
                    ax.vlines(
                        x=abil, ymin=-100, ymax=info, color="black", linestyles="dashed"
            ...
            if cat_highlight is not None and isinstance(items, str):
                ...
                if cat_highlight in range(self.max_score_vector[item] + 1):
                    if cat_highlight == 0:
                        ax.axvspan(
                            -100,
                            self.thresholds_uncentred[item].iloc[0],
                            facecolor="blue",
                            alpha=0.2,
                        )
                    elif cat_highlight == self.max_score_vector[item]:
                        ax.axvspan(
                            self.thresholds_uncentred[item].iloc[-1],
                            100,
                            facecolor="blue",
                            alpha=0.2,
                        )
            ...
            ax.set_ylim(0, y_max)
            ax.set_xlabel(
                "Ability", fontsize=axis_font_size, fontweight="bold", wrap=True
            )
            ax.set_ylabel(
    ...
    def icc(
        ...
        y = np.array(
            [
                self.exp_score_uncentred(a, self.thresholds_uncentred[item])
                for a in abilities
            ]
        ).reshape(-1, 1)
    ...
    def crcs(
        ...
        y = np.array(
            [
                [
                    self.cat_prob_uncentred(a, cat, self.thresholds_uncentred[item])
                    for cat in range(self.max_score_vector[item] + 1)
                ]
                for a in abilities
            ]
    ...
    def iic(
        ...
        y = np.array(
            [
                self.variance_uncentred(a, self.thresholds_uncentred[item])
                for a in abilities
            ]
        ).reshape(-1, 1)
        if ymax is None:
    ...
    def tcc(
        ...
        y = np.array(
            [
                sum(
                    self.exp_score_uncentred(a, self.thresholds_uncentred[it])
                    for it in item_keys
                )
                for a in abilities
            ]
    ...
    def test_info(
        ...
        y = np.array(
            [
                sum(
                    self.variance_uncentred(a, self.thresholds_uncentred[it])
                    for it in item_keys
                )
                for a in abilities
            ]
