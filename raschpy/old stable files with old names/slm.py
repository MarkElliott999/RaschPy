...
class SLM(Rasch):
    ...
    def __init__(self, dataframe, extreme_persons=True, no_of_classes=5, validate=True):
        """
        ...
        if extreme_persons:
            ...
            self.dataframe = dataframe[
                ~dataframe.index.isin(self.invalid_responses.index)
            ]
    ...
    def std_errors(
        ...
        samples = [
            SLM(self.dataframe.sample(frac=1, replace=True), validate=False)
        ...
        self.bootstrap_sample_diffs = {
            f"Sample_{i + 1}": sample.diffs for i, sample in enumerate(samples)
        }

        item_ests = np.array(
            [sample.diffs.loc[self.items].values for sample in samples]
        )

        self.item_se = {
            item: se for item, se in zip(self.items, np.nanstd(item_ests, axis=0))
        }
        ...
        if interval is not None:
            self.item_low = {
                item: low
                for item, low in zip(
                    self.items, np.nanpercentile(item_ests, (1 - interval) * 50, axis=0)
                )
            }
    ...
    def fit_statistics(
        ...
        p = pd.DataFrame(
            ...
            / (
                1
                + np.exp(
                    self.diffs.values[None, :] - self.person_abilities.values[:, None]
                )
            ),
            index=self.persons,
            columns=self.items,
        ...
        if test_stats:
            ...
            psi_var = max(
                0, np.var(self.person_abilities) - (self.rsem_vector**2).mean()
            )
        ...
        self.person_residual_corr = self.std_residual_df.corrwith(
            self.person_abilities.loc[self.std_residual_df.index], axis=0
        )
    ...
    def res_corr_analysis(
        ...
        try:
            ...
            item_labels = list(self.dataframe.columns)
    ...
    def item_stats_df(
        ...
        if hasattr(self, "item_se") == False or (
            interval is not None and hasattr(self, "item_low") == False
        ):
            self.std_errors(
                interval=interval,
                no_of_samples=no_of_samples,
    ...
    def class_intervals(self, items=None, no_of_classes=5):
        ...
        mask_dict[f"class_{no_of_classes}"] = (
            abils >= quantiles.values[no_of_classes - 2]
        )
        for class_no in range(no_of_classes - 2):
            mask_dict[f"class_{class_no + 2}"] = (
                abils >= quantiles.values[class_no]
            ) & (abils < quantiles.values[class_no + 1])

        mean_abilities = {
            class_group: abils[mask_dict[class_group]].mean()
    ...
    def plot_data(
        ...
        if score_lines_item[1] is not None:
            ...
            if all(x > 0 for x in score_lines_item[1]) & all(
                ...
                abils_set = [
                    np.log(score) - np.log(1 - score) + self.diffs[items]
        ...
        if score_lines_test is not None:
            ...
            if all(x > 0 for x in score_lines_test) & all(
                ...
                if items is None:
                    abils_set = [
                        self.score_abil(
                            score, items=self.dataframe.columns, warm_corr=False
                        )
                    ...
                    abils_set = [
                        self.score_abil(score, items=items, warm_corr=False)
                        for score in score_lines_test
                    ]
        ...
        if point_info_lines_item[1] is not None:
            ...
            info_set = [
                self.variance(ability, self.diffs[item])
            ...
            for abil, info in zip(point_info_lines_item[1], info_set):
                ...
                if point_info_labels:
                    plt.text(
                        abil + (x_max - x_min) / 100, y_max / 50, str(round(abil, 2))
                    )
                plt.hlines(
