...
class MFRM(Rasch):
    ...
    Many-Facet Rasch Model (Linacre 1994) with RSM (Andrich 1978) formulation,
        including extended rater representations (Elliott & Buttery 2022a).

        Supports five rater severity parameterisations:
          'global'     — scalar severity λ_r per rater
          'items'      — vector λ_{r,i} per (rater, item)
          'thresholds' — vector λ_{r,k} per (rater, threshold)
          'bivector'   — additive λ_{r,i} + λ_{r,k} per (rater, item, threshold)
                         (rater as RSM; zero-sum threshold vector per rater)
          'matrix'     — full λ_{r,i,k} per (rater, item, threshold)
                         (rater as PCM)
    ...
    def __init__(self, dataframe, max_score=0, extreme_persons=True, no_of_classes=5):
        """
    ...
    def rename_raters_all(self, new_names):
        ...
        if len(new_names) != len(set(new_names)):
            ...
            df_dict = {
                new: self.dataframe.xs(old) for old, new in zip(self.raters, new_names)
            }
    ...
    def rename_person(self, old, new):
        ...
        if old not in self.persons:
            ...
            self.dataframe = self.dataframe.rename(index={old: new}, level=1)
            self.persons = self.dataframe.index.get_level_values(1).unique()
    ...
    def exp_score(
        self, ability, item, difficulties, rater, severities, thresholds, model="global"
    ):
        """
    ...
    def _cat_probs_mfrm(self, abilities, items, raters, thresholds, model, severities):
        """
    ...
    def item_diffs(
        ...
        data = (
            self.dataframe.values.reshape(self.no_of_raters, self.no_of_persons, -1)
            .swapaxes(1, 2)
            .transpose((1, 0, 2))
        )  # (I, R, P)
    ...
    def raters_global(
        ...
        self.severities_global = self.priority_vector(
    ...
    def raters_items(
        ...
        raters_df = pd.DataFrame(
            raters, index=self.raters, columns=self.dataframe.columns
        )
    ...
    def raters_matrix(
        ...
        rater_dict = {
            rater: {
                item: raters[r, j, :] for j, item in enumerate(self.dataframe.columns)
            }
        ...
        self.marginal_severities_thresholds = {
            rater: pd.Series(np.concatenate([[0.0], sev_arr[r, :, :].mean(axis=0)]))
            for r, rater in enumerate(self.raters)
        }
        self.severities_matrix = rater_dict

    def raters_bivector(self, **kw):
        """
        Bivector rater severity estimation.
        ...
        where:
          λ_ri. — mean over thresholds of σ_{r,i,k} (item vector, free mean;
                  overall rater severity lives here)
          λ_r.k — mean over items of σ_{r,i,k}, zero-summed per rater
                  (threshold vector, shape only; Σ_k λ_r.k = 0)

        The reconstructed full matrix λ'_rik is stored as severities_bivector
        in the same {rater: {item: array}} format as severities_matrix, so all
        downstream probability, fit, and plot machinery operates on it without
        modification.

        The intermediate matrix estimates are available as severities_matrix
        but should not be interpreted as a matrix model calibration.

        Public attributes set
        ---------------------
        severities_bivector_items : dict
            {rater: pd.Series({item: float})} — per-(rater, item) marginal means.
        severities_bivector_thresholds : dict
            {rater: pd.Series} of length max_score + 1 — per-(rater, threshold)
            marginal means, zero-summed per rater. Index 0 is always 0.0.
        severities_bivector : dict
            {rater: {item: array}} — reconstructed full severity matrix
            (item_effect + threshold_effect per cell). Used by all downstream
            machinery.
        """
        # Step 1: full matrix PAIR estimation
        self.raters_matrix(**kw)

        # Step 2: bivector vectors are the marginal means computed by
        # raters_matrix() — alias them under bivector names.
        # marginal_severities_thresholds is already zero-summed per rater.
        self.severities_bivector_items = self.marginal_severities_items
        self.severities_bivector_thresholds = self.marginal_severities_thresholds

        # Step 3: reconstruct full matrix as sum of marginals (λ'_rik = λ_ri. + λ_r.k)
        self.severities_bivector = {
            rater: {
                item: np.concatenate(
                    [
                        [0.0],
                        [
                            self.severities_bivector_items[rater][item]
                            + self.severities_bivector_thresholds[rater][k + 1]
                            for k in range(self.max_score)
                        ],
                    ]
                )
    ...
    def calibrate(
        ...
        if constant == 0:
            all_max_items = [
                ...
                for item in self.items
                if (
                    self.dataframe.xs(item, level=-1, axis=1)
                    .dropna(how="all")
                    .eq(self.max_score)
                    .all(axis=None)
                )
        ...
        if len(self.raters) == 1:
            warnings.warn(
                "Only one rater detected. MFRM with a single rater reduces to RSM. "
    ...
    def calibrate_anchor(
        ...
        if model == "global":
            ...
            self._calibrate_anchor_bivector(anchor_raters)
        elif model == "matrix":
    ...
    def _calibrate_anchor_items(self, anchor_raters):
        ...
        self.anchor_severities_items = {
            rater: {item: sev_df.loc[rater, item] for item in self.dataframe.columns}
            for rater in self.raters
        }
    ...
    def _calibrate_anchor_thresholds(self, anchor_raters):
        ...
        self.anchor_thresholds_thresholds[1:] -= self.anchor_thresholds_thresholds[
            1:
        ].mean()
    ...
    def _calibrate_anchor_matrix(self, anchor_raters):
        ...
        sev_array = np.array(
            [
                [self.severities_matrix[rater][item] for item in self.dataframe.columns]
                for rater in self.raters
            ]
        )  # (R, I, K+1)
        ...
        self.anchor_severities_matrix = {
            rater: {
                item: sev_adj[r, j, :] for j, item in enumerate(self.dataframe.columns)
            }
            for r, rater in enumerate(self.raters)
        }
        ...
        self.anchor_marginal_severities_items = {
            rater: sev_df.xs(rater).mean(axis=0) for rater in self.raters
        }
        self.anchor_marginal_severities_thresholds = {
            rater: pd.concat([pd.Series([0.0]), sev_df.xs(rater).mean(axis=1)])
            for rater in self.raters
        }
        for rater in self.raters:
    ...
    def _calibrate_anchor_bivector(self, anchor_raters):
        """
        ...
        vectors.
        """
        self.anchor_diffs_bivector = self.diffs.copy()
        self.anchor_thresholds_bivector = self.thresholds.copy()

        # ---- Item vector adjustment --------------------------------------
        item_sev_df = pd.DataFrame(self.severities_bivector_items).T  # (R, I)
        item_adj = item_sev_df.loc[anchor_raters].mean(axis=0)  # (I,)

        self.anchor_diffs_bivector += item_adj
        item_sev_df -= item_adj
        self.anchor_severities_bivector_items = {
            rater: item_sev_df.loc[rater] for rater in self.raters
        }
        self.anchor_diffs_bivector -= self.anchor_diffs_bivector.mean()

        # ---- Threshold vector adjustment ---------------------------------
        thr_sev_df = pd.DataFrame(self.severities_bivector_thresholds).T  # (R, K+1)
        thr_adj = thr_sev_df.loc[anchor_raters, 1:].mean(axis=0)  # (K,)

        self.anchor_thresholds_bivector[1:] += thr_adj.values
        thr_sev_df.loc[:, 1:] -= thr_adj
        self.anchor_severities_bivector_thresholds = {
            rater: thr_sev_df.loc[rater] for rater in self.raters
        }
        self.anchor_thresholds_bivector[1:] -= self.anchor_thresholds_bivector[
            1:
        ].mean()

        # ---- Reconstruct anchored full matrix ----------------------------
        self.anchor_severities_bivector = {
            rater: {
                item: np.concatenate(
                    [
                        [0.0],
                        [
                            self.anchor_severities_bivector_items[rater][item]
                            + self.anchor_severities_bivector_thresholds[rater][k + 1]
                            for k in range(self.max_score)
                        ],
                    ]
                )
    ...
    def calibrate_bivector_anchor(self, anchor_raters, **kw):
        self.calibrate_anchor("bivector", anchor_raters, **kw)
    ...
    def std_errors(
        ...
        if item_lo is not None:
            self.item_low = pd.Series(item_lo, index=self.dataframe.columns)
            self.item_high = pd.Series(item_hi, index=self.dataframe.columns)
    ...
    def _store_rater_se(self, model, samples, anchor, interval, prefix):
        ...
        if model == "global":
            ...
            rater_ests = np.array([getattr(s, sev_attr).values for s in samples])
            ...
            if interval is not None:
                setattr(
                    ...
                    pd.Series(
                        np.percentile(rater_ests, lo_p, axis=0), index=self.raters
                    ),
                )
                setattr(
                    self,
                    ...
                    pd.Series(
                        np.percentile(rater_ests, hi_p, axis=0), index=self.raters
                    ),
                )
            ...
            se = {rater: rater_ests.xs(rater).std() for rater in self.raters}
            setattr(self, f"{prefix}rater_se_{model}", se)
            if interval is not None:
                setattr(
                    self,
            ...
            sev_attr = (
                f"anchor_severities_thresholds" if anchor else "severities_thresholds"
            )
            rater_ests = np.array(
                [list(getattr(s, sev_attr).values()) for s in samples]
            )
            se = {
                rater: rater_ests[:, r, :].std(axis=0)
                for r, rater in enumerate(self.raters)
            }
            setattr(self, f"{prefix}rater_se_{model}", se)
            if interval is not None:
            ...
            sev_i_attr = (
                "anchor_severities_bivector_items"
                if anchor
                else "severities_bivector_items"
            )
            sev_t_attr = (
                "anchor_severities_bivector_thresholds"
                if anchor
                else "severities_bivector_thresholds"
            )
            # Item vector SEs
            se_items = {
                rater: {
                    item: np.array(
                        [getattr(s, sev_i_attr)[rater][item] for s in samples]
                    ).std()
                    for item in self.dataframe.columns
                }
                for rater in self.raters
            }
            # Threshold vector SEs
            thr_arrs = {
                rater: np.array([getattr(s, sev_t_attr)[rater].values for s in samples])
                for rater in self.raters
            }
            se_thresholds = {
                rater: thr_arrs[rater].std(axis=0) for rater in self.raters
            }
            setattr(self, f"{prefix}rater_se_marginal_items", se_items)
            setattr(self, f"{prefix}rater_se_marginal_thresholds", se_thresholds)
            # No full-matrix rater_se_{model} needed — bivector exposes only vectors
            ...
            by_rater = {
                rater: {i: getattr(s, sev_attr)[rater] for i, s in enumerate(samples)}
                for rater in self.raters
            }
            se = {}
            ...
            for rater in self.raters:
                ...
                item_arrays = {
                    item: np.array([getattr(s, sev_attr)[rater][item] for s in samples])
                    for item in self.dataframe.columns
                }
                se[rater] = {
                    item: item_arrays[item].std(axis=0)
                    for item in self.dataframe.columns
                }

                # Marginal SEs computed from bootstrap samples directly
                # Per-item marginal: mean of λ_{r,i,k} across k, std across bootstrap
                se_marginal_items_rater = {
                    item: np.array(
                        [getattr(s, sev_attr)[rater][item][1:].mean() for s in samples]
                    ).std()
                    for item in self.dataframe.columns
                }
                # Per-threshold marginal: mean of λ_{r,i,k} across i, std across bootstrap
            ...
            setattr(
                self,
                f"{prefix}rater_se_marginal_thresholds",
                se_marginal_thresholds_all,
            )
    ...
    def std_errors_bivector(self, anchor_raters=None, **kw):
        self.std_errors(model="bivector", anchor_raters=anchor_raters, **kw)
    ...
    def anchor_std_errors(
        ...
        anchor_raters : list or None
            Raters whose mean severity is anchored to zero. If None,
            falls back to anchor_raters_{model} set by calibrate_anchor().
        ...
        if stored_flag:
            ...
            anchor_raters_used = getattr(self, f"anchor_raters_{model}", anchor_raters)
            if anchor_raters_used is None:
                raise ValueError(
                    f"anchor_raters must be provided, or calibrate_anchor() "
                    f'must have been run for model="{model}" so that '
                    f"anchor_raters_{model} is available."
                )
            for s in samples:
                s.calibrate_anchor(
                    model,
            ...
            if anchor_raters is None:
                anchor_raters = getattr(self, f"anchor_raters_{model}", None)
            if anchor_raters is None:
                raise ValueError(
                    f"anchor_raters must be provided, or calibrate_anchor() "
                    f'must have been run for model="{model}" so that '
        ...
        # Item difficulty SEs — from anchor_diffs_{model}
        item_ests = np.array(
            [getattr(s, f"anchor_diffs_{model}").values for s in samples]
        )
        item_se, item_lo, item_hi = self._se_from_bootstrap(
        ...
        self.anchor_item_se = pd.Series(item_se, index=self.dataframe.columns)
        if item_lo is not None:
            self.anchor_item_low = pd.Series(item_lo, index=self.dataframe.columns)
            self.anchor_item_high = pd.Series(item_hi, index=self.dataframe.columns)
    ...
    def anchor_std_errors_global(self, anchor_raters=None, **kw):
        self.anchor_std_errors(model="global", anchor_raters=anchor_raters, **kw)

    def anchor_std_errors_items(self, anchor_raters=None, **kw):
        self.anchor_std_errors(model="items", anchor_raters=anchor_raters, **kw)

    def anchor_std_errors_thresholds(self, anchor_raters=None, **kw):
        self.anchor_std_errors(model="thresholds", anchor_raters=anchor_raters, **kw)

    def anchor_std_errors_matrix(self, anchor_raters=None, **kw):
        self.anchor_std_errors(model="matrix", anchor_raters=anchor_raters, **kw)

    def anchor_std_errors_bivector(self, anchor_raters=None, **kw):
        self.anchor_std_errors(model="bivector", anchor_raters=anchor_raters, **kw)
    ...
    def category_probability_dict(
        ...
        if not ext_scores:
            scores = sum(
                person_filter.loc[r].sum(axis=1) * self.max_score for r in self.raters
            )
            total_scores = sum(self.dataframe.loc[r].sum(axis=1) for r in self.raters)
            abilities = abilities[(total_scores > 0) & (total_scores < scores)]
            person_filter = (
                self.dataframe.loc[(slice(None), abilities.index), :]
                .notna()
                .astype(float)
                .replace(0, np.nan)
            )
    ...
    def abil(
        ...
        scores = sum(person_data.loc[r].sum(axis=1) for r in raters).astype(float)
        ext_scores_vec = (
            sum(person_filter.loc[r].sum(axis=1) for r in raters) * self.max_score
        )
        ...
        mean_diffs = (
            sum(
                (person_filter.loc[r] * difficulties.values).sum(axis=1) for r in raters
            )
            / item_count
        )
        ...
        try:
            ...
            warnings.warn(
                f"abil(model={model!r}) failed with exception: {e}. "
    ...
    def abil_global(self, persons, anchor=False, items=None, raters=None, **kw):
        return self.abil(
            persons, model="global", anchor=anchor, items=items, raters=raters, **kw
        )

    def abil_items(self, persons, anchor=False, items=None, raters=None, **kw):
        return self.abil(
            persons, model="items", anchor=anchor, items=items, raters=raters, **kw
        )

    def abil_thresholds(self, persons, anchor=False, items=None, raters=None, **kw):
        return self.abil(
            persons, model="thresholds", anchor=anchor, items=items, raters=raters, **kw
        )

    def abil_matrix(self, persons, anchor=False, items=None, raters=None, **kw):
        return self.abil(
            persons, model="matrix", anchor=anchor, items=items, raters=raters, **kw
        )

    def abil_bivector(self, persons, anchor=False, items=None, raters=None, **kw):
        return self.abil(
            persons, model="bivector", anchor=anchor, items=items, raters=raters, **kw
        )

    def person_abils_global(self, anchor=False, items=None, raters=None, **kw):
        self.person_abils(
            model="global", anchor=anchor, items=items, raters=raters, **kw
        )

    def person_abils_items(self, anchor=False, items=None, raters=None, **kw):
        self.person_abils(
            model="items", anchor=anchor, items=items, raters=raters, **kw
        )

    def person_abils_thresholds(self, anchor=False, items=None, raters=None, **kw):
        self.person_abils(
            model="thresholds", anchor=anchor, items=items, raters=raters, **kw
        )

    def person_abils_matrix(self, anchor=False, items=None, raters=None, **kw):
        self.person_abils(
            model="matrix", anchor=anchor, items=items, raters=raters, **kw
        )

    def person_abils_bivector(self, anchor=False, items=None, raters=None, **kw):
        self.person_abils(
            model="bivector", anchor=anchor, items=items, raters=raters, **kw
        )
    ...
    def warm_global(self, abilities, items, raters, severities, pf, **kw):
        return self.warm(
            abilities, items, raters, severities, self.thresholds, pf, "global"
        )

    def warm_items(self, abilities, items, raters, severities, pf, **kw):
        return self.warm(
            abilities, items, raters, severities, self.thresholds, pf, "items"
        )
    ...
    def warm_matrix(self, abilities, items, raters, severities, pf, **kw):
        return self.warm(
            abilities, items, raters, severities, self.thresholds, pf, "matrix"
        )

    def warm_bivector(self, abilities, items, raters, severities, pf, **kw):
        return self.warm(
            abilities, items, raters, severities, self.thresholds, pf, "bivector"
        )
    ...
    def score_abil(
        ...
        while abs(change) > tolerance and iters <= max_iters:
            result = sum(
                self.exp_score(
                    estimate, item, difficulties, rater, severities, thresholds, model
                )
                for item in items
                for rater in raters
            )
        ...
        if warm_corr:
            ...
            pf_mi = pd.DataFrame(
                ...
                index=pd.MultiIndex.from_product(
                    [raters, ["_score_abil_person_"]], names=self.dataframe.index.names
                ),
                columns=items,
            )
            estimate += float(
    ...
    def score_abil_bivector(self, score, anchor=False, items=None, raters=None, **kw):
        return self.score_abil(score, "bivector", anchor, items, raters, **kw)

    def abil_lookup_table(
        self,
        model="global",
        anchor=False,
        attribute=True,
    ...
    # Backwards-compatible aliases
    def abil_lookup_table_global(self, **kw):
        self.abil_lookup_table(model="global", **kw)

    def abil_lookup_table_items(self, **kw):
        self.abil_lookup_table(model="items", **kw)

    def abil_lookup_table_thresholds(self, **kw):
        self.abil_lookup_table(model="thresholds", **kw)

    def abil_lookup_table_matrix(self, **kw):
        self.abil_lookup_table(model="matrix", **kw)

    def abil_lookup_table_bivector(self, **kw):
        self.abil_lookup_table(model="bivector", **kw)
    ...
    def category_counts_item(self, item, rater=None):
        ...
        if rater not in self.raters:
            warnings.warn(
                f"Invalid rater name: {rater!r}. Returning None.",
                UserWarning,
                stacklevel=2,
            )
            return None
        return (
            self.dataframe.xs(rater)[item]
            .value_counts()
            .reindex(range(self.max_score + 1), fill_value=0)
            .astype(int)
        )
    ...
    def category_counts_df(self):
        ...
        for rater in self.raters:
            ...
            rdf["Missing"] = len(self.dataframe.xs(rater).index) - rdf["Total"]
    ...
    def item_fit_statistics(
        ...
        abils_by_rater = pd.concat(
            {rater: abilities for rater in self.raters}, keys=self.raters
        )
    ...
    def threshold_fit_statistics(self, abilities, diff_df_dict):
        ...
        abil_df = pd.concat(
            [basic_abils_df] * self.no_of_raters, keys=list(self.raters)
        )
        ...
        abil_dev = pd.concat(
            [abilities.loc[self.persons] - abilities.loc[self.persons].mean()]
            * self.no_of_raters,
        ...
        for t in range(self.max_score):
            ...
            for rater in self.raters:
                if model == "global":
                    ...
                    sev_series = pd.Series(severities[rater]).reindex(
    ...
    def rater_pivot(self, df):
        ...
        return pd.DataFrame({rater: df.xs(rater).T.stack() for rater in self.raters})

    def rater_fit_statistics(self, info_df, kurtosis_df, residual_df, std_residual_df):
        ...
        rater_outfit_ms = pd.Series(
            {
                rater: (
                    (std_residual_df**2).xs(rater).sum().sum()
        ...
        rater_infit_ms = pd.Series(
            {
                rater: (
                    (residual_df**2).xs(rater).sum().sum()
    ...
    def rater_fit_statistics_global(self, **kw):
        self._run_rater_fit("global", **kw)

    def rater_fit_statistics_items(self, **kw):
        self._run_rater_fit("items", **kw)

    def rater_fit_statistics_thresholds(self, **kw):
        self._run_rater_fit("thresholds", **kw)

    def rater_fit_statistics_matrix(self, **kw):
        self._run_rater_fit("matrix", **kw)
    ...
    def rater_res_corr_analysis(self, residual_df, std_residual_df):
        ...
        try:
            ...
            pc_labels = [f"PC {pc + 1}" for pc in range(n)]
            eigvec_labels = [f"Eigenvector {pc + 1}" for pc in range(self.no_of_raters)]
            eigenvectors = pd.DataFrame(
                pca.components_[:n, :], index=pc_labels, columns=eigvec_labels
            )
            eigenvalues = pd.DataFrame(
            ...
            warnings.warn(
                "PCA of rater standardised residuals failed. "
                "Eigenvectors and loadings set to None.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def rater_res_corr_analysis_global(self, **kw):
        self._run_rater_res_corr("global", **kw)

    def rater_res_corr_analysis_items(self, **kw):
        self._run_rater_res_corr("items", **kw)

    def rater_res_corr_analysis_thresholds(self, **kw):
        self._run_rater_res_corr("thresholds", **kw)

    def rater_res_corr_analysis_matrix(self, **kw):
        self._run_rater_res_corr("matrix", **kw)

    def rater_res_corr_analysis_bivector(self, **kw):
        self._run_rater_res_corr("bivector", **kw)
    ...
    def _ensure_calibrated(self, model, **kw):
        ...
        if not hasattr(self, f"severities_{model}"):
            self.calibrate(model=model, **calib_kw)
        if anchor_raters is not None:
            stored = getattr(self, f"anchor_raters_{model}", None)
            if (
                not hasattr(self, f"anchor_severities_{model}")
                or stored != anchor_raters
            ):
    ...
    def _ensure_se(
        ...
        if not hasattr(self, trigger) or ci_missing:
            if anc:
                ...
                self.anchor_std_errors(model=model, anchor_raters=anchor_raters)
            else:
                self.std_errors(
                    model=model,
                    interval=interval,
    ...
    def item_stats_df(
        ...
        anc = anchor_raters is not None
        difficulties = getattr(self, f"anchor_diffs_{model}") if anc else self.diffs
        se = (
            self.anchor_item_se
    ...
    def person_stats_df(
        ...
        anc = anchor_raters is not None
        abils = self._get_abils(model, anchor=anc)
        ...
        stats = pd.DataFrame(
            {
                ...
                "Persons": [
                    getattr(self, f"abils_{model}").mean(),
    ...
    def rater_stats_df(
        ...
        if model == "global":
            ...
            stats["Count"] = pd.Series(
                {r: self.dataframe.xs(r).count().sum() for r in self.raters}
            )
            ...
            if model == "items":
                for item in self.items:
                    ...
                    sub["Estimate"] = np.array(
                        [severities[rater][item] for rater in self.raters]
                    ).round(dp)
                    sub["SE"] = np.array(
                        [rse.get(rater, {}).get(item, np.nan) for rater in self.raters]
                    ).round(dp)
                    if interval is not None and rlo is not None:
                        sub[f"{round((1-interval)*50,1)}%"] = np.array(
                            [rlo[rater][item] for rater in self.raters]
                        ).round(dp)
                        sub[f"{round((1+interval)*50,1)}%"] = np.array(
                            [rhi[rater][item] for rater in self.raters]
                        ).round(dp)
                    result[item] = sub.T
                ...
                for t in range(self.max_score):
                    ...
                    sub = pd.DataFrame(index=self.raters)
                    sub["Estimate"] = np.array(
                        [severities[rater][t + 1] for rater in self.raters]
                    ).round(dp)
                    sub["SE"] = np.array(
                        [
                            rse.get(rater, np.zeros(self.max_score + 1))[t + 1]
                            for rater in self.raters
                        ]
                    ).round(dp)
                    if interval is not None and rlo is not None:
                        sub[f"{round((1-interval)*50,1)}%"] = np.array(
                            [rlo[rater][t + 1] for rater in self.raters]
                        ).round(dp)
                        sub[f"{round((1+interval)*50,1)}%"] = np.array(
                            [rhi[rater][t + 1] for rater in self.raters]
                        ).round(dp)
                    result[key] = sub.T
                ...
                lo_mg_i = getattr(
                    self,
                    (
                        "anchor_rater_low_marginal_items"
                        if anc
                        else "rater_low_marginal_items"
                    ),
                    None,
                )
                hi_mg_i = getattr(
                    self,
                    (
                        "anchor_rater_high_marginal_items"
                        if anc
                        else "rater_high_marginal_items"
                    ),
                    None,
                )
                lo_mg_t = getattr(
                    self,
                    (
                        "anchor_rater_low_marginal_thresholds"
                        if anc
                        else "rater_low_marginal_thresholds"
                    ),
                    None,
                )
                hi_mg_t = getattr(
                    self,
                    (
                        "anchor_rater_high_marginal_thresholds"
                        if anc
                        else "rater_high_marginal_thresholds"
                    ),
                    None,
                )
                ...
                for item in self.items:
                    sub = pd.DataFrame(index=self.raters)
                    sub["Estimate"] = np.array(
                        [mg_items[rater][item] for rater in self.raters]
                    ).round(dp)
                    sub["SE"] = np.array(
                        [
                            mg_se_i.get(rater, {}).get(item, np.nan)
                            for rater in self.raters
                        ]
                    ).round(dp)
                    if interval is not None and lo_mg_i is not None:
                        sub[f"{round((1-interval)*50,1)}%"] = np.array(
                            [lo_mg_i[rater][item] for rater in self.raters]
                        ).round(dp)
                        sub[f"{round((1+interval)*50,1)}%"] = np.array(
                            [hi_mg_i[rater][item] for rater in self.raters]
                        ).round(dp)
                    result[item] = sub.T
                ...
                for t in range(self.max_score):
                    ...
                    sub = pd.DataFrame(index=self.raters)
                    sub["Estimate"] = np.array(
                        [mg_thrs[rater][t + 1] for rater in self.raters]
                    ).round(dp)
                    sub["SE"] = np.array(
                        [
                    ...
                    if interval is not None and lo_mg_t is not None:
                        sub[f"{round((1-interval)*50,1)}%"] = np.array(
                            [lo_mg_t[rater][t + 1] for rater in self.raters]
                        ).round(dp)
                        sub[f"{round((1+interval)*50,1)}%"] = np.array(
                            [hi_mg_t[rater][t + 1] for rater in self.raters]
                        ).round(dp)
                    result[key] = sub.T
                ...
                if marginal:
                    mg_i_attr = (
                        f"anchor_marginal_severities_items"
                        if anc
                        else "marginal_severities_items"
                    )
                    mg_t_attr = (
                        f"anchor_marginal_severities_thresholds"
                        if anc
                        else "marginal_severities_thresholds"
                    )
                    mg_items = getattr(self, mg_i_attr)
                    mg_thrs = getattr(self, mg_t_attr)
                    mg_se_i = getattr(
                    ...
                    for item in self.items:
                        for t in range(self.max_score):
                            ...
                            sub["SE"] = np.array(
                                [
                                    rse.get(rater, {}).get(
                                        item, np.zeros(self.max_score + 1)
                                    )[t + 1]
                                    for rater in self.raters
                                ]
                            ).round(dp)
                            if interval is not None and rlo is not None:
                                sub[f"{round((1-interval)*50,1)}%"] = np.array(
                                    [rlo[rater][item][t + 1] for rater in self.raters]
                                ).round(dp)
                                sub[f"{round((1+interval)*50,1)}%"] = np.array(
                                    [rhi[rater][item][t + 1] for rater in self.raters]
                                ).round(dp)
                            result[key] = sub.T
    ...
    def _save_residuals_for(self, model, which, filename, **kw):
        ...
        if not hasattr(self, attr):
            runner = (
                self._run_item_res_corr if which == "item" else self._run_rater_res_corr
            )
            runner(model, **kw)
        self.save_residuals(
    ...
    def _class_masks(abils, no_of_classes):
        ...
        q = abils.quantile([(i + 1) / no_of_classes for i in range(no_of_classes - 1)])
        mask = {
            "class_1": abils < q.values[0],
            f"class_{no_of_classes}": abils >= q.values[-1],
            **{
                f"class_{i + 2}": ((abils >= q.values[i]) & (abils < q.values[i + 1]))
    ...
    def _severity_item_offset(self, model, severities, rater):
        ...
        if model == "global":
            return pd.Series(
                (
                    float(severities.loc[rater].iloc[0])
                    if isinstance(severities.loc[rater], pd.Series)
                    else float(severities.loc[rater])
                ),
                index=self.items,
            )
        elif model == "items":
            ...
            return pd.Series(
                {item: np.mean(severities[rater][item][1:]) for item in self.items}
            )
    ...
    def _zero_severities(self, model, severities):
        ...
        if model == "global":
            ...
            return {r: {it: 0.0 for it in sev} for r, sev in severities.items()}
        elif model == "thresholds":
            return {r: np.zeros_like(arr) for r, arr in severities.items()}
        elif model in ("bivector", "matrix"):
            return {
                r: {it: np.zeros_like(arr) for it, arr in item_dict.items()}
                for r, item_dict in severities.items()
            }

    def _mean_severities(self, model, severities, rater):
        """Return a severities structure where `rater` has the mean severity
        across all raters.  Used to plot curves that match obs averaged across
        the full rater pool."""
        if model == "global":
            mean_val = float(severities.mean())
            result = severities.copy()
            result[rater] = mean_val
            return result
        elif model == "items":
            mean_by_item = {
                it: np.mean([severities[r][it] for r in severities])
                for it in next(iter(severities.values()))
            }
            result = {r: dict(sev) for r, sev in severities.items()}
            result[rater] = mean_by_item
            return result
        elif model == "thresholds":
            arrs = list(severities.values())
            mean_arr = np.mean(arrs, axis=0)
            result = {r: arr.copy() for r, arr in severities.items()}
            result[rater] = mean_arr
            return result
        elif model in ("bivector", "matrix"):
            items = list(next(iter(severities.values())).keys())
            mean_by_item = {
                it: np.mean([severities[r][it] for r in severities], axis=0)
                for it in items
            }
            result = {
                r: {it: arr.copy() for it, arr in item_dict.items()}
                for r, item_dict in severities.items()
            }
            result[rater] = mean_by_item
            return result
    ...
    def class_intervals(
        self, abilities, items=None, raters=None, shift=0, no_of_classes=5
    ):
        ...
        mean_abilities = (
            pd.Series({cg: abils[mask_dict[cg]].mean() for cg in class_groups}) - shift
        )
    ...
    def class_intervals_cats(
        ...
        # Build raw ability structure (no difficulty/severity subtracted) for
        # reporting x-positions, using mask_abils only for quantile binning.
        raw_abil_df = pd.concat(
            {
                r: pd.DataFrame({it: abilities for it in self.dataframe.columns})
                for r in self.raters
            },
            keys=self.raters,
        )
        if item is None and rater is None:
            pf = self.dataframe.notna().astype(float).replace(0, np.nan)
            raw_for_x = (raw_abil_df * pf).unstack().unstack()
        elif item is None and rater is not None:
            df_r2 = self.dataframe.xs(rater)
            pf2 = df_r2.notna().astype(float).replace(0, np.nan)
            raw_for_x = (raw_abil_df.xs(rater) * pf2).unstack()
        elif item is not None and rater is None:
            df_i2 = self.dataframe[item].unstack(level=0)
            pf2 = df_i2.notna().astype(float).replace(0, np.nan)
            raw_for_x = (raw_abil_df[item].unstack(level=0) * pf2).unstack()
        else:
            df_ri2 = self.dataframe.xs(rater)[item]
            pf2 = df_ri2.notna().astype(float).replace(0, np.nan)
            raw_for_x = raw_abil_df.xs(rater)[item] * pf2
    ...
    def plot_data(
        ...
        if isinstance(raters, str):
            raters = None if raters in ("none", "zero", "all") else [raters]
        if isinstance(items, str):
        ...
        with plt.rc_context({"font.family": font, "font.size": axis_font_size}):
            ...
            if point_info_lines_item[1] is not None:
                ...
                r = (
                    raters[0]
                    if isinstance(raters, list)
                    else raters if raters is not None else list(self.raters)[0]
                )
                for abil in point_info_lines_item[1]:
                    info = self.variance(
                        abil, item, difficulties, r, severities, thresholds, model
                    )
                    ax.vlines(
                        x=abil, ymin=-100, ymax=info, color="black", linestyles="dashed"
                    )
                    ax.hlines(
                        y=info, xmin=-100, xmax=abil, color="black", linestyles="dashed"
                    )
                    if score_labels:
                        ax.text(
                            abil + (x_max - x_min) / 100,
            ...
            if point_info_lines_test is not None:
                ...
                for abil in point_info_lines_test:
                    info = sum(
                        self.variance(
                            abil, it, difficulties, r, severities, thresholds, model
                        )
                        for it in item_keys
                        for r in rater_list
                    )
            ...
            if point_csem_lines is not None:
                ...
                for abil in point_csem_lines:
                    ...
                    csem = 1.0 / (info**0.5)
                    ax.vlines(
                        x=abil, ymin=-100, ymax=csem, color="black", linestyles="dashed"
                    )
                    ax.hlines(
                        y=csem, xmin=-100, xmax=abil, color="black", linestyles="dashed"
                    )
                    if score_labels:
                        ax.text(
                            abil + (x_max - x_min) / 100,
    ...
    def icc(
        ...
        r_use = rater if rater is not None else list(self.raters)[0]
        # When no specific rater requested, average exp_score across all raters
        # to match the obs y-values which are mean scores across the rater pool.
        if rater is None:
            all_raters = list(self.raters)
            y = np.array(
                [
                    np.mean(
                        [
        ...
        if obs is not None:
            abil_attr = f'{"anchor_" if anchor else ""}abils_{model}'
            if not hasattr(self, abil_attr):
                self.person_abils(model=model, anchor=anchor)
            person_abils = getattr(self, abil_attr)
            xobsdata, yobsdata = self.class_intervals(
                person_abils, items=item, raters=rater, no_of_classes=no_of_classes
            )
    ...
    def crcs(
        ...
        r_use = (
            rater[0]
            if isinstance(rater, list)
            else rater if rater is not None else list(self.raters)[0]
        )
        diff_use = 0.0 if item is None else float(difficulties[item])
        # Use mean severities for the neutral curve so it represents a
        # typical rater, matching obs proportions averaged across all raters.
        sev_for_curve = (
            severities
            if rater is not None
        ...
        if obs is not None:
            ...
            if not hasattr(self, abil_attr):
                self.person_abils(model=model, anchor=anchor)
            abilities = getattr(self, abil_attr)
    ...
    def threshold_ccs(
        ...
        # Neutral threshold CCS: for a neutral plot (no specific rater
        # requested) use zero severity so thresholds are placed at diff + tau.
        # For the thresholds model with a specific rater, each threshold has its
        # own rater severity — use per-threshold values rather than the mean.
        if rater is None:
            sev_thresh = np.zeros(self.max_score)
        elif model == "thresholds":
            sev_thresh = np.array(
                [float(severities[r_use][t + 1]) for t in range(self.max_score)]
            )
        elif model in ("bivector", "matrix"):
            item_key = item if item is not None else list(self.items)[0]
            sev_thresh = np.array(
                [
                    float(severities[r_use][item_key][t + 1])
                    for t in range(self.max_score)
                ]
            )
        else:
        ...
        abs_thresh = thresholds[1:] + diff_shift + sev_thresh
        ...
        if obs is not None:
            ...
            # - rater=None: severity subtracted → need to add it back
            # - rater specified: nothing subtracted → no shift needed
            ...
            if rater is None:
                ...
                # For rater-specific plot: mean_abs is raw ability, no shift needed.
    ...
    def iic(
        ...
        r_use = (
            ...
            if isinstance(rater, list)
                ...
                rater
                if rater is not None and rater not in ("none", "zero")
    ...
    def tcc(
        ...
        rater_list = (
            list(self.raters)
            if raters is None
            else ([raters] if isinstance(raters, str) else raters)
        )
        ...
        if obs:
            ...
            df_sub = (
                self.dataframe.loc[pd.IndexSlice[rater_list, :], item_keys]
                if item_keys != list(self.items)
                else self.dataframe.loc[pd.IndexSlice[rater_list, :], :]
            )

            # TCC obs: restrict to persons with complete data across all
            # rater×item combinations in scope, so all totals share the same
            ...
            if n_complete == 0:
                warnings.warn(
                    ...
                    "complete data across all rater×item combinations in scope.",
                    UserWarning,
                    stacklevel=2,
                )
                xobsdata = yobsdata = np.array(np.nan)
                ...
                if n_complete < len(obs_counts):
                    warnings.warn(
                        ...
                        f"all rater×item combinations in scope.",
                        UserWarning,
                        stacklevel=2,
                    )
                abil_index = person_abils.index
                total_scores = df_sub.sum(axis=1).groupby(level=1).sum()
                total_scores = (
                    total_scores.reindex(complete_persons).reindex(abil_index).dropna()
                )
                abils_aligned = person_abils.reindex(total_scores.index)

                class_groups = [f"class_{i + 1}" for i in range(no_of_classes)]
                quantiles = abils_aligned.quantile(
                    [(i + 1) / no_of_classes for i in range(no_of_classes - 1)]
                )
                mask_dict = {
                    "class_1": abils_aligned < quantiles.values[0],
                    f"class_{no_of_classes}": abils_aligned >= quantiles.values[-1],
                }
                for i in range(no_of_classes - 2):
                    mask_dict[f"class_{i + 2}"] = (
                        abils_aligned >= quantiles.values[i]
                    ) & (abils_aligned < quantiles.values[i + 1])

                xobsdata = pd.Series(
                    {cg: abils_aligned[mask_dict[cg]].mean() for cg in class_groups}
                )
                yobsdata = pd.Series(
        ...
        # axis with no severity adjustment.  When no specific raters are
        # requested, remove each rater's severity contribution from the curve
        # so it also represents a zero-severity baseline.
        # Curve: total expected score summed across all rater×item combinations.
        y = np.array(
            [
                sum(
                    self.exp_score(
