...
class RSM(Rasch):
    ...
    def __init__(
        self, dataframe, max_score=None, extreme_persons=True, no_of_classes=5
    ):
    ...
    def std_errors(
        ...
        """
        samples = [
            RSM(self.dataframe.sample(frac=1, replace=True), max_score=self.max_score)
        ...
        self.item_bootstrap = pd.DataFrame(
            item_ests, index=sample_idx, columns=self.dataframe.columns
        )
        self.item_se = pd.Series(
            np.nanstd(item_ests, axis=0), index=self.dataframe.columns
        )

        self.threshold_bootstrap = pd.DataFrame(
            threshold_ests, index=sample_idx, columns=range(self.max_score + 1)
        )
        self.threshold_se = np.nanstd(threshold_ests, axis=0)

        # Category width bootstrap: width_k = tau_{k+1} - tau_k
        cat_widths = {
            cat + 1: threshold_ests[:, cat + 2] - threshold_ests[:, cat + 1]
        ...
        if interval is not None:
            ...
            self.item_low = pd.Series(
                np.percentile(item_ests, lo, axis=0), index=self.dataframe.columns
            )
            self.item_high = pd.Series(
                np.percentile(item_ests, hi, axis=0), index=self.dataframe.columns
            )
    ...
    def category_counts_df(self):
        ...
        """
        cat_counts = {
            item: self.category_counts_item(item) for item in self.dataframe.columns
        }
    ...
    def fit_statistics(
        ...
        exp_score = (cats_arr[:, None, None] * probs).sum(axis=0)  # (N, I)
        self.exp_score_df = (
            pd.DataFrame(exp_score, index=abilities.index, columns=self.items)
            * missing_mask
        )
        ...
        self.info_df = (
            pd.DataFrame(info, index=abilities.index, columns=self.items) * missing_mask
        )
        self.info_df[degenerate] = np.nan

        self.kurtosis_df = (
            pd.DataFrame(kurtosis, index=abilities.index, columns=self.items)
            * missing_mask
        )
        ...
        if trim_cat_prob_dict:
            ...
            self.cat_prob_dict = {
                cat: pd.DataFrame(
                    probs[cat], index=abilities.index, columns=self.items
                ).loc[df.index]
        ...
        abil_df = pd.DataFrame(
            np.tile(self.person_abilities.values[:, None], (1, self.no_of_items)),
        ...
        for t in range(self.max_score):
            ...
            dich = self.dataframe.where(self.dataframe.isin([t, t + 1]), np.nan) - t
            ...
            diff_df = pd.DataFrame(
                np.tile(
                    self.diffs.values + self.thresholds[t + 1],
        ...
        if test_stats:
            ...
            self.psi = (
                (np.var(self.person_abilities) - mean_rsem2) / mean_rsem2
            ) ** 0.5
    ...
    def threshold_stats_df(
        ...
        if interval is not None:
            stats[f"{round((1 - interval) * 50, 1)}%"] = self.threshold_low[1:].round(
                dp
            )
            stats[f"{round((1 + interval) * 50, 1)}%"] = self.threshold_high[1:].round(
                dp
            )
    ...
    def class_intervals_thresholds(self, item=None, no_of_classes=5):
        ...
        def make_masks(abils):
            ...
            md = {
                ...
                **{
                    f"class_{i + 2}": (
                        (abils >= q.values[i]) & (abils < q.values[i + 1])
                    )
                    for i in range(no_of_classes - 2)
                },
            }
        ...
        with plt.rc_context({"font.family": font, "font.size": axis_font_size}):
            ...
            if point_info_lines_test is not None:
                ...
                for abil in point_info_lines_test:
                    info = sum(
                        self.variance(abil, self.diffs[it], self.thresholds)
                        for it in item_keys
                    )
                    ax.vlines(
                        x=abil, ymin=-100, ymax=info, color="black", linestyles="dashed"
            ...
            if items is not None and cat_highlight in range(self.max_score + 1):
                if cat_highlight == 0:
                    ax.axvspan(
                        -100,
                        self.diffs[items] + self.thresholds[1],
                        facecolor="blue",
                        alpha=0.2,
                    )
                elif cat_highlight == self.max_score:
                    ax.axvspan(
                        self.diffs[items] + self.thresholds[self.max_score],
                        100,
                        facecolor="blue",
                        alpha=0.2,
                    )
    ...
    def icc(
        ...
        y = np.array(
            [self.exp_score(a, self.diffs[item], self.thresholds) for a in abilities]
        ).reshape(-1, 1)
    ...
    def threshold_ccs(
        ...
        y = np.array(
            [[1.0 / (1.0 + np.exp(thr - a)) for thr in abs_thresh] for a in abilities]
        )
    ...
    def iic(
        ...
        abilities = np.arange(-20, 20, 0.1)
        y = np.array(
            [self.variance(a, self.diffs[item], self.thresholds) for a in abilities]
        ).reshape(-1, 1)
        if ymax is None:
    ...
    def tcc(
        ...
        y = np.array(
            [
                sum(
                    self.exp_score(a, self.diffs[it], self.thresholds)
                    for it in item_keys
                )
                for a in abilities
            ]
    ...
    def test_info(
        ...
        y = np.array(
            [
                sum(
                    self.variance(a, self.diffs[it], self.thresholds)
                    for it in item_keys
                )
                for a in abilities
            ]
