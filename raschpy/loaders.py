...
def _read_file(filename, header, index_col):
    ...
    f = filename.lower()
    if f.endswith('.xlsx'):
        return pd.read_excel(filename, sheet_name=0, header=header,
...
def _mfrm_reindex_and_stack(responses_dict, item_ids, scores):
    ...
    for rater in rater_names:
        responses_dict[rater] = (responses_dict[rater]
                                 .reindex(all_persons)
    ...
    full_idx = pd.MultiIndex.from_product([rater_names, all_persons],
                                          names=['Rater', 'Person'])
...
def loadup_pcm(filename,
    ...
    is_valid = (responses % 1 == 0) & (responses >= 0) & responses.le(max_score_vector, axis=1)
...
def loadup_mfrm_single(filename,
    ...
    if f.endswith('.xlsx'):
        responses = pd.read_excel(filename, sheet_name=0, header=header,
                                  index_col=[0, 1], engine='openpyxl')
    else:
        responses = pd.read_csv(filename, header=header, index_col=[0, 1])

    if long:
        responses = pd.pivot_table(responses, values='Score',
                                   index=['Rater', 'Person'],
                                   columns='Item', aggfunc='min', dropna=False)
    ...
    full_idx    = pd.MultiIndex.from_product([rater_names, persons],
                                             names=['Rater', 'Person'])
...
def loadup_mfrm_xlsx_tabs(filename,
    ...
    if long:
        ...
        combined.index.names = ['Rater', 'Person']
        combined = pd.pivot_table(combined, values='Score',
                                  index=['Rater', 'Person'],
                                  columns='Item', aggfunc='min', dropna=False)
        sheets = {rater: combined.xs(rater)
    ...
    for rater, df in sheets.items():
        if missing is not None:
            missing_list = ([missing] if isinstance(missing, (str, int, float))
                            else list(missing))
