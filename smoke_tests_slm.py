...
smoke_tests_slm.py
------------------
Smoke tests for the SLM model.

Checks:
  1. Simulate -> instantiate -> calibrate (bail on failure)
  2. item_stats_df, person_stats_df, test_stats_df — non-empty, no all-NaN cols
  3. person_abils — returns Series, no all-NaN
  4. std_errors — no crash
  5. All plot methods via Agg backend

Run with:
    python smoke_tests_slm.py
    python smoke_tests_slm.py -v
"""
...
def main():
    parser = argparse.ArgumentParser(description="SLM smoke tests")
